...
return [
    ...
    'key'                          => function ($throttle, $request) {
        ...
        $whitelist = [
            '45.201.141.234',
            // '127.0.0.1', // Uncomment for local testing
        ];

        if (in_array($clientIp, $whitelist)) {
            return null; // Return null to disable throttling for whitelisted IPs
        }
        return true; // Return true to use default IP-based throttling
    },
    ...
    'visit_fail_response'          => function (Throttle $throttle, Request $request, int $wait_seconds) {
        return Response::create([
            'code' => 0,
            'msg'  => '请求过于频繁，请在 ' . $wait_seconds . ' 秒后重试。',
        ], 'json')->header([
