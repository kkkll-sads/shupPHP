...
export default class baTable {
     ...
     */
    exportToExcel = () => {
        if (this.runBefore('exportToExcel') === false) return
        ...
        try {
            ...
            // 获取显示的列作为表头（排除选择框和操作按钮）
            this.table.column.forEach((col: any) => {
                if (col.prop && col.label && col.type !== 'selection' && col.render !== 'buttons' && col.show !== false) {
                ...
                this.table.data.forEach((row: any) => {
                    ...
                    this.table.column.forEach((col: any) => {
                        if (col.prop && col.label && col.type !== 'selection' && col.render !== 'buttons' && col.show !== false) {
                            ...
                            // 处理特殊渲染
                            if (col.render === 'tag' && col.replaceValue) {
                                // 状态标签 - 使用 replaceValue 映射
                                if (col.replaceValue && col.replaceValue[String(value)]) {
                                    value = col.replaceValue[String(value)]
                                }
                            } else if (col.render === 'datetime') {
                                // 时间格式化
                                if (value) {
            ...
            this.runAfter('exportToExcel', { filename })
        } catch (error) {
            ...
            this.runAfter('exportToExcel', { error })
        }
    }
