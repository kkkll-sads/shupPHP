<template>
    <div class="default-main ba-table-box">
        ...
        <TableHeader
            ...
            :quick-search-placeholder="'手机号 / 备注'"
        >
            <!-- 单个手机号快速添加 -->
            <div class="single-mobile-input">
                <el-input
                    v-model="singleMobile"
                    placeholder="输入手机号"
                    style="width: 140px;"
                    maxlength="11"
                    @keyup.enter="addSingleMobile"
                />
                <el-button v-blur type="success" @click="addSingleMobile" :loading="addingMobile">
...
defineOptions({
    name: 'user/mobileWhitelist',
})

const adminInfo = useAdminInfo()
const uploadRef = ref()
const singleMobile = ref('')
const addingMobile = ref(false)

// 添加单个手机号
const addSingleMobile = () => {
    const mobile = singleMobile.value.trim()
    if (!mobile) {
    ...
    addingMobile.value = true
    createAxios({
        url: '/admin/user.MobileWhitelist/add',
        method: 'post',
        data: {
            mobile: mobile,
            status: 1,
...
const handleFileChange = (file: any) => {
    ...
    createAxios({
        url: '/admin/user.MobileWhitelist/import',
        method: 'post',
        data: formData,
    })
        .then((res) => {
...
const batchEnable = () => {
    ...
    ElMessageBox.confirm(`确定要启用选中的 ${ids.length} 条记录吗？`, '提示', {
        ...
        createAxios({
            url: '/admin/user.MobileWhitelist/batchStatus',
            method: 'post',
            data: {
                ids: ids,
                status: 1,
...
const baTable = new baTableClass(
    new baTableApi('/admin/user.MobileWhitelist/'),
    {
        column: [
