        ...
        <el-scrollbar v-loading="baTable.form.loading" class="ba-table-form-scrollbar">
            ...
            >
                ...
                >
                    ...
                    <el-form-item prop="price" label="可用金额单价">
                        <el-input-number
                            ...
                            placeholder="请输入可用金额价格"
                        />
                        <span style="margin-left: 10px; color: #999;">元</span>
                    </el-form-item>

                    <el-form-item prop="score_price" label="消费金数量">
                        <el-input-number
                            v-model="baTable.form.items!.score_price"
                            :min="0"
                            controls-position="right"
                            placeholder="请输入消费金数量，0表示仅可用金额购买"
                        />
                        <span style="margin-left: 10px; color: #999;">消费金</span>
                    </el-form-item>
                    ...
                    <FormItem
                        ...
                        :input-attr="{
                            border: true,
                            content: { 'money': '余额购买', 'score': '消费金购买', 'both': '组合支付' },
                            disabled: true  // 自动根据价格设置，不可手动修改
                        }"
                    >
                        <template #append>
                            <el-alert
                                ...
                                <template #title>
                                    <span style="font-size: 12px;">购买方式自动设置：<br>
                                    • 仅设置可用金额单价：仅余额购买<br>
                                    • 仅设置消费金数量：仅消费金购买<br>
                                    • 同时设置两种价格：强制组合支付<br>
                                    • 不可手动修改，由系统自动管理
...
// 监听价格变化，自动调整购买方式
watch([() => baTable.form.items?.price, () => baTable.form.items?.score_price], ([newPrice, newScorePrice]) => {
    if (baTable.form.items) {
        const price = parseFloat(newPrice || 0)
        const scorePrice = parseInt(newScorePrice || 0)

        // 根据价格设置自动调整购买方式
        if (price > 0 && scorePrice > 0) {
            // 同时设置了余额和消费金，自动设为both（两者都可）
            baTable.form.items.purchase_type = 'both'
        } else if (price > 0 && scorePrice === 0) {
            // 只设置了余额，设为money
            baTable.form.items.purchase_type = 'money'
        } else if (price === 0 && scorePrice > 0) {
            // 只设置了消费金，设为score
            baTable.form.items.purchase_type = 'score'
        }
        // 如果都没有设置，保持原有值不变
    }
})
...
const rules = {
    ...
    price: [{ required: true, message: '可用金额单价不能为空', trigger: 'change' }],
    score_price: [{ required: true, message: '消费金数量不能为空', trigger: 'change' }],
