                ...
                <template v-if="item.show !== false">
                    ...
                    >
                        ...
                        <template v-if="typeof item.render === 'function'" #default="scope">
                            <span>{{ item.render(scope.row, item, scope.column, scope.$index) }}</span>
                        </template>
                        <!-- 预定义字段渲染器 -->
                        <template v-else-if="item.render && item.render !== 'slot'" #default="scope">
                            <component
                                :row="scope.row"
                                :field="item"
                                :column="scope.column"
...
// 确保关键渲染器存在
if (!fieldRenderer['datetime']) {
    console.warn('datetime renderer not found, trying manual import')
    import('./fieldRender/datetime.vue').then(module => {
        fieldRenderer['datetime'] = module.default
    })
}
if (!fieldRenderer['buttons']) {
    console.warn('buttons renderer not found, trying manual import')
    import('./fieldRender/buttons.vue').then(module => {
        fieldRenderer['buttons'] = module.default
    })
}
