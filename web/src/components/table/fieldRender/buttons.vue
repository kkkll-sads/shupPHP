<template>
    <div v-memo="[field, row]">
        <template v-for="(btn, idx) in buttonList" :key="idx">
...
const buttonList = computed(() => {
    if (!props.field.buttons) {
        return []
    }
    if (typeof props.field.buttons === 'function') {
        return (props.field.buttons as (row: TableRow) => OptButton[])(props.row) || []
    }
    return props.field.buttons || []
})
