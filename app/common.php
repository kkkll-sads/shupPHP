...
if (!function_exists('full_url')) {
     ...
     * @return string
     */
    function toFullUrl(string $path): string
    {
        return full_url($path, false);
    }
}
...
if (!function_exists('get_score_exchange_green_power_rate')) {
    /**
     * 获取消费金兑换绿色算力的比例
     * @return float 兑换比例，如果未配置则返回0
     */
    function get_score_exchange_green_power_rate(): float
    {
        // 优先从基准值计算比例
        $scoreBase = get_sys_config('score_green_score_base');
        $greenPowerBase = get_sys_config('score_green_power_base');
        
        if ($scoreBase !== null && $scoreBase !== '' && $greenPowerBase !== null && $greenPowerBase !== '' 
            && (float)$scoreBase > 0 && (float)$greenPowerBase > 0) {
            // 使用基准值计算比例
            return (float)$scoreBase / (float)$greenPowerBase;
        }
        
        // 兼容旧配置：直接读取比例
        $configValue = get_sys_config('score_green_rate');
        if ($configValue === null || $configValue === '') {
            return 0;
        }
        $rate = (float)$configValue;
        return $rate > 0 ? $rate : 0;
    }
}

if (!function_exists('calculate_weight_from_power')) {
    /**
     * 根据算力计算权重
     * @param float $power 算力值
     * @return int 权重值，如果未配置则返回0
     */
    function calculate_weight_from_power(float $power): int
    {
        // 优先从基准值计算比例
        $powerBase = get_sys_config('power_weight_power_base');
        $weightBase = get_sys_config('power_weight_weight_base');
        
        if ($powerBase !== null && $powerBase !== '' && $weightBase !== null && $weightBase !== '' 
            && (float)$powerBase > 0 && (float)$weightBase > 0) {
            // 使用基准值计算比例
            $rate = (float)$weightBase / (float)$powerBase;
        } else {
            // 兼容旧配置：直接读取比例
            $configRate = get_sys_config('power_weight_rate');
            if ($configRate === null || $configRate === '' || (float)$configRate <= 0) {
                return 0;
            }
            $rate = (float)$configRate;
        }
        
        return (int)floor($power * $rate);
    }
}

if (!function_exists('calculate_power_from_weight')) {
    /**
     * 根据权重计算需要的算力
     * @param int $weight 权重值
     * @return float 需要的算力值，如果未配置则返回0
     */
    function calculate_power_from_weight(int $weight): float
    {
        // 优先从基准值计算比例
        $powerBase = get_sys_config('power_weight_power_base');
        $weightBase = get_sys_config('power_weight_weight_base');
...
if (!function_exists('generateFlowNo')) {
    /**
     * 生成唯一的流水号
     * 格式: FL{timestamp}{random6}
     * @return string
     */
    function generateFlowNo(): string
    {
        return 'FL' . time() . str_pad(mt_rand(1, 999999), 6, '0', STR_PAD_LEFT);
    }
}

if (!function_exists('generateSJSFlowNo')) {
    /**
     * 生成SJS开头的流水号（用于确权金、专项金等业务）
     * 格式: SJS{timestamp}{user_id}{random4}
...
if (!function_exists('generateBatchNo')) {
    /**
     * 生成批次号
     * @param string $prefix 前缀（如 CONSIGN, ORDER等）
