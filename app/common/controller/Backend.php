...
class Backend extends Api
{
    ...
    public function queryOrderBuilder()
    {
        ...
        if ($order && is_string($order)) {
            ...
            $order = [];
            // 如果只有两个部分，且第二个部分是 asc/desc，则认为是 'field,desc' 格式
            if (count($orderParts) == 2 && !str_contains($orderParts[0], ' ') && in_array(strtolower(trim($orderParts[1])), ['asc', 'desc'])) {
                $order[trim($orderParts[0])] = strtolower(trim($orderParts[1]));
            } else {
                // 处理多个字段排序，如 'sort desc,id desc'
                foreach ($orderParts as $part) {
                    $part = trim($part);
                    if (str_contains($part, ' ')) {
                        $parts = explode(' ', $part, 2);
                        $order[$parts[0]] = strtolower($parts[1] ?? 'asc');
                    } else {
                        $order[$part] = 'asc';
                    }
                }
        ...
        if (!$this->orderGuarantee) {
            ...
            $orderGuaranteeParts = explode(',', $this->orderGuarantee);
            // 如果只有两个部分，且第二个部分是 asc/desc，则认为是 'field,desc' 格式
            if (count($orderGuaranteeParts) == 2 && !str_contains($orderGuaranteeParts[0], ' ') && in_array(strtolower(trim($orderGuaranteeParts[1])), ['asc', 'desc'])) {
                $this->orderGuarantee = [trim($orderGuaranteeParts[0]) => strtolower(trim($orderGuaranteeParts[1]))];
            } else {
                // 处理多个字段排序，如 'sort desc,id desc'
                $this->orderGuarantee = [];
                foreach ($orderGuaranteeParts as $part) {
                    $part = trim($part);
                    if (str_contains($part, ' ')) {
                        $parts = explode(' ', $part, 2);
                        $this->orderGuarantee[$parts[0]] = strtolower($parts[1] ?? 'asc');
                    } else {
                        $this->orderGuarantee[$part] = 'asc';
                    }
                }
            }
        }
