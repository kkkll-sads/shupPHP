...
/**
 * 充值奖励算力补发脚本
 * 用法: php retroactive_recharge_reward.php [日期，格式：Y-m-d，默认今天]
 */
...
if ($rewardPowerRate <= 0) {
    echo "错误: 充值奖励比例未配置或为0，请在活动奖励配置中设置 recharge_reward_power_rate\n";
    exit(1);
}

echo "奖励比例: " . $rewardPowerRate . "%\n\n";
...
echo "找到 " . count($orders) . " 个已通过的充值订单\n\n";

$needRetroactive = [];
$alreadyRewarded = [];

// 检查哪些订单需要补发（通过 extra 字段中的 order_id 判断）
// 注意：只检查是否存在记录，不检查数量，避免重复补发
// JSON格式中 order_id 后面有空格："order_id": 16409
...
echo "已有奖励记录的订单: " . count($alreadyRewarded) . " 个\n";
echo "需要补发的订单: " . count($needRetroactive) . " 个\n\n";

if (empty($needRetroactive)) {
    echo "所有订单都已发放奖励，无需补发\n";
    exit(0);
}
...
foreach ($needRetroactive as $order) {
    try {
        ...
        if ($rewardPower <= 0) {
            Db::rollback();
            echo "警告: 订单 " . $order['order_no'] . " 奖励算力为0，跳过\n";
            $failCount++;
            continue;
        }
        ...
        if (!$user) {
            Db::rollback();
            echo "错误: 订单 " . $order['order_no'] . " 用户不存在，跳过\n";
            $failCount++;
            continue;
        }
        ...
        echo "✓ 订单 " . $order['order_no'] . " 补发成功：+" . $rewardPower . " 算力（用户ID: " . $order['user_id'] . "，充值金额: " . $order['amount'] . "元）\n";
        
    } catch (\Throwable $e) {
        Db::rollback();
        $failCount++;
        echo "✗ 订单 " . $order['order_no'] . " 补发失败：" . $e->getMessage() . "\n";
    }
}

echo "\n=== 补发完成 ===\n";
echo "成功: " . $successCount . " 个订单\n";
echo "失败: " . $failCount . " 个订单\n";
