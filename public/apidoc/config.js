  ...
  window.fetch = function(...args) {
    ...
    if (typeof url === 'string' && url.includes('apiDetail')) {
      ...
      if (!urlObj.searchParams.get('path')) {
        // 尝试从存储中获取path
        const path = localStorage.getItem('apidoc_current_menu_key') ||
                    sessionStorage.getItem('apidoc_current_menu_key');
        if (path) {
          urlObj.searchParams.set('path', path);
          args[0] = urlObj.toString();
          console.log('拦截并修复apiDetail请求，添加path参数:', path);
        }
      }
    }
      ...
      if (url.includes('apiDetail')) {
        ...
        if (!urlObj.searchParams.get('path')) {
          ...
          if (path) {
            urlObj.searchParams.set('path', path);
            url = urlObj.toString();
            console.log('拦截XHR并修复apiDetail请求，添加path参数:', path);
          }
        }
      }
...
window.apidocFeConfig = {
  ...
  HTTP: {
    ...
    HOSTS: [{
      title:"",
      host: window.location.origin + "/index.php"
    }],
    // 请求前缀地址
    API_PREFIX:"/apidoc",
    // 接口响应超时时间
  ...
  // 接口详情页的tab顺序
  API_DETAIL_TABS: ['table', 'json', 'ts', 'curl', 'debug'],
  // 接口详情表格属性
  API_TABLE_PROPS: {
    // 是否默认展开所有行
  ...
  DEBUG_EVENTS: {
    ...
    // 修复apiDetail请求的path参数
    fixApiDetailPath(param) {
      return new Promise((resolve, reject) => {
        const { config } = param
        // 如果是apiDetail请求且没有path参数
        if (config.url.includes('apiDetail') && !config.params.path) {
          // 尝试从页面URL或其他地方获取path
          const urlParams = new URLSearchParams(window.location.search)
          const hashParams = new URLSearchParams(window.location.hash.split('?')[1] || '')
          const path = urlParams.get('path') || hashParams.get('path')
          if (path) {
            config.params.path = path
            param.message = '自动添加path参数: ' + path
          } else {
            reject('无法获取path参数，请刷新页面后重试')
          }
        }
        resolve(param)
      })
  ...
  // （选配）自定义方法
  CUSTOM_METHODS: {
    // 全局请求拦截器，修复apiDetail请求缺少path参数的问题
    beforeRequest: function(config) {
      // 如果是apiDetail请求且没有path参数
      if (config.url && config.url.includes('apiDetail') && config.params && !config.params.path) {
        // 尝试从localStorage、sessionStorage或其他地方获取当前的menuKey
        const currentMenuKey = localStorage.getItem('apidoc_current_menu_key') ||
                              sessionStorage.getItem('apidoc_current_menu_key');

        if (currentMenuKey) {
          config.params.path = currentMenuKey;
          console.log('自动添加path参数:', currentMenuKey);
        } else {
          console.warn('apiDetail请求缺少path参数，且无法从存储中获取');
        }
      }
      return config;
    },
    ...
    onMenuClick: function(menuItem) {
      // 当点击菜单项时，保存menuKey到存储中
      if (menuItem && menuItem.menuKey) {
        localStorage.setItem('apidoc_current_menu_key', menuItem.menuKey);
        sessionStorage.setItem('apidoc_current_menu_key', menuItem.menuKey);
        console.log('保存menuKey到存储:', menuItem.menuKey);
      }
    }
  },
  // 多语言
