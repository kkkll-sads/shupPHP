...
class Terminal
{
    ...
    public static function getCommand(string $key): bool|array
    {
        ...
        if (stripos($key, '.')) {
            ...
            // 支持两层嵌套（如 finance.daily）
            if ($keyCount == 2) {
                if (!array_key_exists($keyParts[0], $commands) || !is_array($commands[$keyParts[0]]) || !array_key_exists($keyParts[1], $commands[$keyParts[0]])) {
                    return false;
                }
                $command = $commands[$keyParts[0]][$keyParts[1]];
            }
            // 支持三层嵌套（如 collection.mining.check）
            elseif ($keyCount == 3) {
                if (!array_key_exists($keyParts[0], $commands) 
                    || !is_array($commands[$keyParts[0]]) 
                    || !array_key_exists($keyParts[1], $commands[$keyParts[0]])
                    || !is_array($commands[$keyParts[0]][$keyParts[1]])
                    || !array_key_exists($keyParts[2], $commands[$keyParts[0]][$keyParts[1]])) {
                    return false;
                }
                $command = $commands[$keyParts[0]][$keyParts[1]][$keyParts[2]];
            } else {
                return false;
            }
        } else {
        ...
        if (!is_array($command)) {
            ...
            // 如果 cwd 不存在，设置为空字符串（默认使用 root_path）
